...
import { NOTIFY_DISPATCH_QUEUE, emitNotification } from './notify';
...
describe('emitNotification', () => {
  it('sends to NOTIFY_DISPATCH_QUEUE', async () => {
    const boss = makeBoss();
    const event: NotifyEvent = { kind: 'status-changed', from: 'preparing', to: 'executing' };
  ...
  it('payload contains jobId, seq, event, and emittedAt', async () => {
    ...
    const event: NotifyEvent = { kind: 'status-changed', from: 'publishing', to: 'completed' };
    await emitNotification(boss as never, 'job-42', 7, event);
    const after = Date.now();

    const [, payload] = boss.send.mock.calls[0] as [
      string,
      { jobId: string; seq: number; emittedAt: string; event: NotifyEvent },
    ];
  ...
  it('forwards different event kinds unchanged', async () => {
    const boss = makeBoss();
    const chunkEvent: NotifyEvent = { kind: 'chunk', raw: 'some output' };
